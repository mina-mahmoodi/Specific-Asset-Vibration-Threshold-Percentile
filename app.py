    ...
    if ready_to_process:
        for uploaded_file in uploaded_files:
            ...
            try:
                ...
                if motor_state_available:
                    ...
                    if df_on.empty:
                        st.warning(
                            ...
                            f"Assuming all states are ON and using all non-zero data."
                        )
                        df_use = df_combined
                    else:
                        st.success(
                            f"✅ Motor ON data found in '{uploaded_file.name}' sheet '{selected_sheet}'. "
                            f"Calculating thresholds based on motor ON data."
                        )
                        df_use = df_on
                    ...
                    if df_combined.empty:
                        ...
                        st.warning(
                            ...
                            f"Calculating thresholds based on all available non-zero data."
                        )
                        df_use = df_combined
                ...
                st.success(
                    f"✅ Using {row_count} rows of non-zero vibration data from '{uploaded_file.name}' sheet '{selected_sheet}' "
                    f"between **{start_time.strftime('%Y-%m-%d %H:%M:%S')}** and **{end_time.strftime('%Y-%m-%d %H:%M:%S')}** "
