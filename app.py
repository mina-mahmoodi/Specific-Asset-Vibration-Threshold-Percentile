...
if uploaded_file:
    ...
    col_map = {
        ...
        'Timestamp': 'timestamp',  # Add your timestamp column name here
        'Time': 'timestamp',       # Add alternative timestamp names if any
        'Date': 'timestamp'
    }
    ...
    if 'timestamp' not in df.columns:
        st.error("Missing timestamp column. Please make sure your data contains a timestamp column named 'timestamp' or similar.")
    else:
        ...
        if not all(col in df.columns for col in required_cols):
            ...
            # Ensure motor_state column exists
            if 'motor_state' not in df.columns:
                # If no motor_state column, assign all as ON (3)
                df['motor_state'] = 3
            else:
                # Fill missing motor_state by forward fill to propagate known states through time
                df['motor_state'] = df['motor_state'].fillna(method='ffill')

                # Any remaining NaNs (timestamps with no motor_state) set to 3 (ON)
                df['motor_state'] = df['motor_state'].fillna(3)

            # Now filter only rows with motor_state == 3
